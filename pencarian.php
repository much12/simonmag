                ...
                <div class="row">
                    <div class="container">
                        <center><br>
                            ...
                            if (isset($_POST['cari'])) {
                                $cari = $_POST['cari'];
                                echo "Hasil pencarian : <b>" . $cari . "</b><br><br>";
                                include "koneksi.php";
                                $cek = mysqli_query($koneksi, "select * from t_peserta where peserta_nim = '$cari'");
                                $row = mysqli_num_rows($cek);

                                if ($row > 0) {
                                    $data = mysqli_fetch_array($cek);
                                    if ($data['peserta_status'] == '2') {
                            ?>
                                        <div class="alert alert-warning" role="alert">
                                            Pengajuan anda masih dalam proses pengecekan oleh Admin <b>DISKOMINFO PROVINSI KALIMANTAN SELATAN</b>.
                                        </div>
                                    <?php } elseif ($data['peserta_status'] == '3') {
                                    ?>
                                        <div class="alert alert-danger" role="alert">
                                            Mohon maaf pengajuan anda ditolak, terimakasih telah berpartisipasi sebagai pendaftar peserta magang di <b>DISKOMINFO PROVINSI KALIMANTAN SELATAN</b>.
                                        </div>
                                    <?php } else { ?>
                                        <div class="alert alert-success" role="alert">
                                            Selamat anda telah diterima sebagai peserta magang di <b>DISKOMINFO PROVINSI KALIMANTAN SELATAN</b>.
                                        </div>
                                    <?php
                                    }
                                } else {
                                    ?> <p>Tidak menemukan data calon peserta.</p> <?php
                                                                                }
                                                                            } else {
                                                                            } ?>
                            <center>
